---
title: "WAP_E2_Simple_Regression_Analyses"
...
ggsave("Fig_S4_E2_Recall_and_Beliefs.pdf", plot = e1_recall_and_belief_plots, width = 6.75, height = 9, units = "in", dpi = 320, device = "pdf")
ggsave("Fig_S4_E2_Recall_and_Beliefs.png", plot = e1_recall_and_belief_plots, width = 6.75, height = 9, units = "in", dpi = 320, device = "png")
```
...
ggsave("Fig_S5_E2_Change_Classifications.pdf", plot = rc_all, width = 8, height = 5, units = "in", dpi = 320)
ggsave("Fig_S5_E2_Change_Classifications.png", plot = rc_all, width = 8, height = 5, units = "in", dpi = 320)
...
item_contrasts <- summary(pairs(emmeans(rc_model, ~ ItemType * Correction_Class, type = "response"))) #display contrasts from emmeans
...
ann_text_class_1a <- data.frame(ItemType = 2, prob = c(.98, .50, .13), lab = c("Misinformation\nRecollected", "Correction\nRemembered", "Correction\nNot Remembered"), ResponseType = "Fact Recall")
ann_text_class_1b <- data.frame(ItemType = 4, prob = c(.99, .66, .19), lab = c("Misinformation\nRecollected", "Correction\nRemembered", "Correction\nNot Remembered"), ResponseType = "Fact Recall")
ann_text_class_2a <- data.frame(ItemType = c(2.45, 2.45, 2), prob = c(.03, .14, .39), lab = c("Misinformation\nRecollected", "Correction\nRemembered", "Correction\nNot Remembered"), ResponseType = "Misinformation Intrusions")
ann_text_class_2b <- data.frame(ItemType = c(4.60, 4.60, 4), prob = c(.00, .10, .30), lab = c("Misinformation\nRecollected", "Correction\nRemembered", "Correction\nNot Remembered"), ResponseType = "Misinformation Intrusions")

#Create data frames for line segments
panel1a <- data.frame(x = 1.15, y = .519, xend = 1.9, yend = c(.25, .58, .83), ResponseType = "Fact Recall")
panel1b <- data.frame(x = 3.15, y = .676, xend = 3.9, yend = c(.32, .75, .85), ResponseType = "Fact Recall")
panel2a <- data.frame(x = 1.15, y = .167, xend = 1.9, yend = c(.05, .13, .27), ResponseType = "Misinformation Intrusions")
panel2b <- data.frame(x = 3.15, y = .084, xend = c(3.9, 4.0, 3.9), yend = c(.03, .07, .19), ResponseType = "Misinformation Intrusions")

#Plot List 1 correct recall for all responses and A-B, A-D conditionalized on change recollection
e2_recall_cond <- ggplot(recall_intrusion_bubble_df, aes(x = ItemType, y = prob, color = Correction_Class))+
...
belief_cond_df$lower.CL <- ifelse(belief_cond_df$lower.CL < 2, 2, belief_cond_df$lower.CL)
belief_cond_df$upper.CL <- ifelse(belief_cond_df$upper.CL > 5.85, 5.85, belief_cond_df$upper.CL)
...
```{r fig.height=5, fig.width=10}
#create data frames text labels
ann_text_class_1a <- data.frame(ItemType = c(2.00, 2.60, 2.00), prob = c(5.65, 4.85, 3.80), lab = c("Misinformation\nRecollected", "Correction\nRemembered", "Correction\nNot Remembered"), ResponseType = "Fact Recall")
ann_text_class_1b <- data.frame(ItemType = c(4.00, 4.60, 4.60), prob = c(5.85, 5.25, 4.15), lab = c("Misinformation\nRecollected", "Correction\nRemembered", "Correction\nNot Remembered"), ResponseType = "Fact Recall")
ann_text_class_2a <- data.frame(ItemType = c(1.55, 2.55, 2.52), prob = c(3.40, 3.55, 4.90), lab = c("Misinformation\nRecollected", "Correction\nRemembered", "Correction\nNot Remembered"), ResponseType = "Misinformation Intrusions")
ann_text_class_2b <- data.frame(ItemType = c(3.55, 4.20, 4.60), prob = c(3.10, 5.05, 3.50), lab = c("Misinformation\nRecollected", "Correction\nRemembered", "Correction\nNot Remembered"), ResponseType = "Misinformation Intrusions")

#Create data frames for line segments
panel1a <- data.frame(x = 1.15, y = 4.84, xend = c(1.9, 2.0, 1.9), yend = c(4.26, 4.88, 5.14), ResponseType = "Fact Recall")
panel1b <- data.frame(x = 3.15, y = 5.28, xend = c(4.1, 4.1, 3.85), yend = c(4.60, 5.10, 5.35), ResponseType = "Fact Recall")
panel2a <- data.frame(x = 1.15, y = 4.19, xend = c(1.95, 1.90, 1.85), yend = c(4.32, 3.62, 3.78), ResponseType = "Misinformation Intrusions")
panel2b <- data.frame(x = 3.15, y = 4.07, xend = c(3.95, 3.90, 3.85), yend = c(3.97, 4.11, 3.40), ResponseType = "Misinformation Intrusions")

#bubble plot
e2_belief_cond <- ggplot(belief_cond_df, aes(x = ItemType, y = prob, color = Correction_Class)) +
                  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL, x = x_pos), position = position_dodge(), size = .4, width = 0, show.legend = FALSE) +
                  geom_point(aes(size = prop, x = x_pos)) +
                  geom_segment(data = panel1a, aes(x=x,y=y,xend=xend,yend=yend),color="black",size = .25, arrow=arrow(length=unit(0.2,"cm")))+
                  geom_segment(data = panel1b, aes(x=x,y=y,xend=xend,yend=yend),color="black",size = .25, arrow=arrow(length=unit(0.2,"cm")))+
                  ...
                  facet_grid(. ~ ResponseType) +
                  scale_x_continuous(breaks=seq(1, 4, 1), limits = c(.5, 5), labels=c("Correction", "Correction\n(Conditional)", "Correction +\nReminder", "Correction +\nReminder\n(Conditional)")) +
                  scale_y_continuous(name="Belief Rating",limits=c(1.95, 6), breaks=seq(2, 6, .5))+
                  labs(x = "Statement Type") +
                  theme(legend.key=element_rect(fill="transparent",size=0),
                        legend.key.height=unit(1.2,"cm"),
                        legend.key.width=unit(.8,"cm"),
...
e2_cond_recall_and_belief_plots <- grid.arrange(e2_recall_cond, e2_belief_cond, nrow = 2)

e2_cond_recall_and_belief_plots

ggsave("Fig_S6_E2_Conditional_Analyses.pdf", plot = e2_cond_recall_and_belief_plots, width = 10, height = 10, units = "in", dpi = 320)
ggsave("Fig_S6_E2_Conditional_Analyses.png", plot = e2_cond_recall_and_belief_plots, width = 10, height = 10, units = "in", dpi = 320)
