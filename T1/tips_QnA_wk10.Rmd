...
```{r}
library(readxl)
study4 <- read_excel(here("T1", "data", "3_pfattheicher", "Study4.xlsx")) %>%
  rowid_to_column("ID") %>%
  zap_labels()
...
```{r}
data_1_raw = read_csv(here("T1", "data", "4_tybur", "WTR_Comfort_S1.csv"))
data_2 = read_csv(here("T1", "data", "4_tybur", "WTR_Comfort_S2.csv"))
data_3 = read_csv(here("T1", "data", "4_tybur", "WTR_Comfort_S3.csv"))
...
```{r}
# how to round
COVID <- read_sav(here("T1", "data","8_smith",  "believing.sav")) %>%
  rowid_to_column("ID")
